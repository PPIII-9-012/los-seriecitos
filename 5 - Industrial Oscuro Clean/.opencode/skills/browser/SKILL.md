...
**IMPORTANT**: All scripts are located in `.opencode/skills/browser/scripts/` and must be called with full paths.

## Start Chrome

```bash
.opencode/skills/browser/start.js              # Fresh profile
.opencode/skills/browser/start.js --profile    # Copy your profile (cookies, logins)
```
...
```bash
.opencode/skills/browser/nav.js https://example.com
.opencode/skills/browser/nav.js https://example.com --new
```
...
```bash
.opencode/skills/browser/eval.js 'document.title'
.opencode/skills/browser/eval.js 'document.querySelectorAll("a").length'
```
...
```bash
.opencode/skills/browser/screenshot.js
```
...
```bash
.opencode/skills/browser/pick.js "Click the submit button"
```
