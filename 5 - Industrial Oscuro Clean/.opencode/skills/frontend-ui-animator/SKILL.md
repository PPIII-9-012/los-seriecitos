...
## Technical Stack & Dedicated Skills

Use the appropriate specialized skill for each library:

| Library | Skill | When to Use |
|---------|-------|-------------|
| **Framer Motion** | `use_skill("motion-framer")` | React gesture/layout animations, variants, AnimatePresence |
| **GSAP** | `use_skill("gsap-scrolltrigger")` | Timeline sequences, ScrollTrigger, production scroll animations |
| **React Spring** | `use_skill("react-spring-physics")` | Physics-based spring animations, gesture-driven motion |
| **Anime.js** | `use_skill("animejs")` | Lightweight JS animations, timeline chaining, SVG morphing |
| **Lottie** | `use_skill("lottie-animations")` | After Effects animations, complex vector motion, lightweight JSON |
| **Scroll Reveal** | `use_skill("scroll-reveal-libraries")` | AOS, ScrollReveal.js, IntersectionObserver-based reveals |

**Default approach**: Use CSS animations first. Only invoke the dedicated skill when the animation requires that library's specific capabilities.
